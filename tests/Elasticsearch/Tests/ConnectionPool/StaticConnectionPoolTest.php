...
namespace Elasticsearch5\Tests\ConnectionPool;
...
class StaticConnectionPoolTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $mockConnection = m::mock('\Elasticsearch5\Connections\Connection')
                          ->shouldReceive('ping')
                          ->andReturn(true)
                          ->getMock()
        ...
        $selector = m::mock('\Elasticsearch5\ConnectionPool\Selectors\RoundRobinSelector')
                    ->shouldReceive('select')
                    ->andReturn($connections[0])
                    ->getMock();

        $connectionFactory = m::mock('\Elasticsearch5\Connections\ConnectionFactory');

        $randomizeHosts = false;
        $connectionPool = new Elasticsearch5\ConnectionPool\StaticConnectionPool($connections, $selector, $connectionFactory, $randomizeHosts);
    ...
    /**
     * @expectedException Elasticsearch5\Common\Exceptions\NoNodesAvailableException
     */
    ...
    public function testAllExceptLastHostFailPingRevivesInSkip()
    {
        ...
        $goodConnection = m::mock('\Elasticsearch5\Connections\Connection')
    ...
    {
        $clientBuilder = \Elasticsearch5\ClientBuilder::create();
        ...
        } catch (Elasticsearch5\Common\Exceptions\NoNodesAvailableException $e) {
            // All good
        } catch (\Exception $e) {
            throw $e;
        }
