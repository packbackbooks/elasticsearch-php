...
namespace Elasticsearch5\Tests\ConnectionPool\Selectors;
...
class RoundRobinSelectorTest extends \PHPUnit_Framework_TestCase
{
     ...
     *
     * @covers \Elasticsearch5\ConnectionPool\Selectors\RoundRobinSelector::select
     *
     * @return void
     */
    public function testTenConnections()
    {
        $roundRobin = new Elasticsearch5\ConnectionPool\Selectors\RoundRobinSelector();

        $mockConnections = array();
        foreach (range(0, 10) as $index) {
            $mockConnections[$index] = $this->getMockBuilder('\Elasticsearch5\Connections\CurlMultiConnection')
